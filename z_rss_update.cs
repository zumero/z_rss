	...
	class Program
	{
        ...
        private static void do_feeds(SQLiteConnection db_all_feeds, List<feed_row> rows)
        {
            foreach (feed_row q in rows)
            {
                ...
                if (f != null)
                {
                    // set the feed title if it's not there already

                    db_all_feeds.Execute("INSERT OR IGNORE INTO about (feedid, title) VALUES (?,?)",
                            q.feedid,
                            f.Title.Text
                            );
                    ...
                    string result = db_this_feed.ExecuteScalar<string>(
                            @"SELECT zumero_sync('main', 'https://zinst393e9343b87.s.zumero.net', ?);", dbfile_name_for_this_feed);
		...
		public static void Main (string[] args)
		{
            ...
            // find ALL the feeds that have never been updated

            rows = db_all_feeds.Query<feed_row> ("SELECT f.feedid AS feedid, f.url AS url FROM feeds AS f LEFT OUTER JOIN last_update AS u ON (f.feedid=u.feedid) WHERE (u.when_unix_time IS NULL);");
